...
type TabKey = 'umum' | 'keamanan' | 'integrasi' | 'backup' | 'shortcuts';
...
const tabDefs: { key: TabKey; label: string; icon: string }[] = [
    ...
    },
    {
        key: 'integrasi',
        label: 'Integrasi',
        icon: 'M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.54a4.5 4.5 0 00-6.364-6.364L4.5 8.25l4.5 4.5m0-4.5l4.5 4.5',
    },
    {
        key: 'backup',
        label: 'Backup',
...
export default function PengaturanIndex() {
    ...
    const integrasiForm = useForm({
        ...
        google_drive_folder_id: pengaturan?.google_drive_folder_id || '',
    });
    ...
    function submitIntegrasi(e: FormEvent) {
        e.preventDefault();
        integrasiForm.post('/pengaturan', { forceFormData: true });
    }
                            ...
                            <div className="space-y-6">
                                <div className="rounded-xl border border-border bg-muted/30 p-5">
                                    <div className="mb-4 flex items-center justify-between">
                                        <h3 className="font-semibold text-foreground">Google Drive Integration</h3>
                                        {pengaturan?.google_refresh_token ? (
                                    ...
                                    <p className="mb-4 text-sm text-muted-foreground">
                                        Hubungkan akun Google untuk export laporan ke Google Sheets secara otomatis.
                                    </p>

                                    <form onSubmit={submitIntegrasi} className="space-y-4">
                                        <div>
                                            <label className="mb-1.5 block text-sm font-medium text-foreground">Google Client ID</label>
                                            <input
                                                type="text"
                                                value={integrasiForm.data.google_client_id}
                                                onChange={(e) => integrasiForm.setData('google_client_id', e.target.value)}
                                                className={inputClass}
                                                placeholder="Masukkan Google Client ID"
                                            />
                                            {integrasiForm.errors.google_client_id && (
                                                <p className="mt-1 text-xs text-destructive">{integrasiForm.errors.google_client_id}</p>
                                            )}
                                        </div>

                                        <div>
                                            <label className="mb-1.5 block text-sm font-medium text-foreground">Google Client Secret</label>
                                            <input
                                                type="text"
                                                value={integrasiForm.data.google_client_secret}
                                                onChange={(e) => integrasiForm.setData('google_client_secret', e.target.value)}
                                                className={inputClass}
                                                placeholder="Masukkan Google Client Secret"
                                            />
                                            {integrasiForm.errors.google_client_secret && (
                                                <p className="mt-1 text-xs text-destructive">{integrasiForm.errors.google_client_secret}</p>
                                            )}
                                        </div>

                                        <div>
                                            <label className="mb-1.5 block text-sm font-medium text-foreground">Google Drive Folder ID (opsional)</label>
                                            <input
                                                type="text"
                                                value={integrasiForm.data.google_drive_folder_id}
                                                onChange={(e) => integrasiForm.setData('google_drive_folder_id', e.target.value)}
                                                className={inputClass}
                                                placeholder="ID folder di Google Drive untuk menyimpan file export"
                                            />
                                            {integrasiForm.errors.google_drive_folder_id && (
                                                <p className="mt-1 text-xs text-destructive">{integrasiForm.errors.google_drive_folder_id}</p>
                                            )}
                                            <p className="mt-1.5 text-xs text-muted-foreground">
                                                Kosongkan jika ingin menyimpan di root Google Drive.
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3 pt-2">
                                            <button
                                                type="submit"
                                                disabled={integrasiForm.processing}
                                                className="rounded-lg bg-primary px-6 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
                                            >
                                                {integrasiForm.processing ? 'Menyimpan...' : 'Simpan Konfigurasi'}
                                            </button>
                                        </div>
                                    </form>
                                </div>
                                ...
                                {/* Authorize Button */}
                                {(pengaturan?.google_client_id && pengaturan?.google_client_secret) && (
                                    <div className="rounded-xl border border-border bg-muted/30 p-5">
                                        <h3 className="mb-2 font-semibold text-foreground">Otorisasi Akun Google</h3>
                                        <p className="mb-4 text-sm text-muted-foreground">
                                            {pengaturan?.google_refresh_token
                                                ? 'Akun Google sudah terhubung. Klik tombol di bawah jika ingin menghubungkan ulang.'
                                                : 'Klik tombol di bawah untuk menghubungkan akun Google Anda.'}
                                        </p>
                                        <a
                                            href="/pengaturan/google/authorize"
                                        ...
                                        >
                                            ...
                                            {pengaturan?.google_refresh_token ? 'Re-otorisasi Google' : 'Otorisasi Google'}
                                        </a>
                                    </div>
                                )}
                            </div>
